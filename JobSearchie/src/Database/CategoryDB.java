...
public class CategoryDB implements DBHelper {
    ...
    public CategoryDB(Connection conn) throws SQLException {
        insertCategory = conn.prepareStatement(Insert.CATEGORY, Statement.RETURN_GENERATED_KEYS);
        queryCategory = conn.prepareStatement(Query.CATEGORY, Statement.RETURN_GENERATED_KEYS);
        queryCategoryId = conn.prepareStatement(Query.CATEGORY_ID, Statement.RETURN_GENERATED_KEYS);
    }
    ...
    public static String getCategory(int id) {
        try {
            ...
            if (results.next()) {
                return results.getString(Column.CATEGORY);
            } else
                return null;
    ...
    public static int getCategoryId(String category) {
        try {
            ...
            if (results.next()) {
                return results.getInt(Column.ID);
            } else
                return -1;
        } catch (SQLException e) {
    ...
    public static class Query {
        public static final String CATEGORY_ID = "SELECT " + ID + " FROM " + NAME + " WHERE " + Column.CATEGORY + " = ?";
        public static final String CATEGORY = "SELECT " + Column.CATEGORY + " FROM " + NAME + " WHERE " + ID + " = ?";
    }
    ...
    public static class Insert {
        public static final String CATEGORY = "INSERT INTO " + NAME + " (" + Column.CATEGORY + ") VALUES (?)";
    }
