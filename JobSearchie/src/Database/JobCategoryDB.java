...
public class JobCategoryDB implements DBHelper {
    ...
    public JobCategoryDB(Connection conn) throws SQLException {
        queryJobCategories = conn.prepareStatement(Query.JOB_category, Statement.RETURN_GENERATED_KEYS);
        insertJobCategory = conn.prepareStatement(Insert.JOB_category, Statement.RETURN_GENERATED_KEYS);
    }
    ...
    private ArrayList<Integer> getJobCategoryIds(int jobId) {
        ...
        try {
            ...
            while (results.next()) {
                categoryIds.add(results.getInt(Column.categoryID));
            }
