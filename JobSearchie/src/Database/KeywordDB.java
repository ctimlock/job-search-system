...
public class KeywordDB implements DBHelper {
    ...
    public KeywordDB(Connection conn) throws SQLException {
        insertKeyword = conn.prepareStatement(Insert.INSERT_KEYWORD, Statement.RETURN_GENERATED_KEYS);
        queryKeyword = conn.prepareStatement(Query.KEYWORD, Statement.RETURN_GENERATED_KEYS);
        queryKeywordId = conn.prepareStatement(Query.KEYWORD_ID, Statement.RETURN_GENERATED_KEYS);
    }
    ...
    public static String getKeyword(int id) {
        try {
            ...
            if (results.next()) {
                return results.getString(Column.KEYWORD);
            } else
                return null;
