...
public class UserDB implements DBHelper {
    ...
    public UserDB(Connection conn) throws SQLException {
        queryUserAccountTypeByEmail = conn.prepareStatement(Query.ACCOUNTTYPE_BY_EMAIL);
        queryUserByEmail = conn.prepareStatement(Query.USER_BY_EMAIL);
        insertIntoAdmin = conn.prepareStatement(Insert.ADMIN);
        insertIntoJobSeeker = conn.prepareStatement(Insert.JOBSEEKER);
        insertIntoRecruiter = conn.prepareStatement(Insert.RECRUITER);
    }
    ...
    public String getUserAccountType(String email) {
        try {
            ...
            if (result.next())
                return result.getString(Column.ACCOUNTTYPE);
            else
                return null;
        } catch (SQLException e) {
