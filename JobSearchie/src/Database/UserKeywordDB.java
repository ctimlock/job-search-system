...
public class UserKeywordDB implements DBHelper {
    ...
    public UserKeywordDB(Connection conn) throws SQLException {
        queryUserKeywords = conn.prepareStatement(Query.USER_KEYWORD, Statement.RETURN_GENERATED_KEYS);
        insertUserKeyword = conn.prepareStatement(Insert.USER_KEYWORD, Statement.RETURN_GENERATED_KEYS);
    }
    ...
    private ArrayList<Integer> getUserKeywordIds(User user) {
        ...
        try {
            ...
            while (results.next()) {
                keywordIds.add(results.getInt(Column.KEYWORDID));
            }
